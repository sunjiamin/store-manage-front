    ...
    <Row>
      <Col>
        <Card>
          ...
          <Row class="operation">
            <Button @click="addClass" type="primary" icon="plus-round">添加客户</Button>
  ...
  export default {
    ...
    methods: {
      ...
      getCustomerList() {
        // 多条件搜索用户列表
        this.loading = true;
        this.getRequest("/customer/getByCondition", this.searchForm).then(res => {
          this.loading = false;
          if (res.success === true) {
          ...
          this.$Modal.confirm({
            ...
            onOk: () => {
              this.$refs.exportTable.exportCsv({
                filename: "用户数据."+util.getCurrentDatetime2()
              });
            }
          });
      ...
      remove(v) {
        this.$Modal.confirm({
          title: "确认删除",
          content: "您确认要删除客户 " + v.name + " ?",
          onOk: () => {
            this.deleteRequest("/customer/delByIds", { ids: v.id }).then(res => {
              if (res.success === true) {
      ...
      delAll() {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            this.deleteRequest("/customer/delByIds", { ids: ids }).then(res => {
              if (res.success === true) {
                this.$Message.success("删除成功");
                this.init();
              }
